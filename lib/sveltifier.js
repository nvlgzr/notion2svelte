...
import { slugForId, setSlugForId } from './slug-map.js'
...
`,
  warning: (callingContext, ref, message) => `<p style="display: inline-block; background: floralwhite; padding: 1rem; margin: 0.25rem 0; border: 1px fuchsia solid; border-radius: 6px; ">⚠️ ${callingContext}: <span style="font-weight: 600; display: inline; padding: 3px; border-radius: 3px;">${ref}.</span>&nbsp;${message}</p>`
}
export default h;
...
function enrich(span) {
  ...
  const k = (t, url) => {
    if (url.match(/^\/[a-f0-9]{32}$/)) {
      ...
      return slug
        ? `<a href="${slug}">${t}</a>`
        : h.warning("k(t, url)", t, `No slug found for page id ${noLeadingSlash}`)
    } else {
      return url ? `<a href="${url}">${t}</a>` : h.warning("k(t, url)", t, `No URL provided for ${t}`)
    }
  }
...
{:else }
  ...
  <h1>Dis Page</h1>
  <pre>{JSON.stringify(${JSON.stringify(page, (k, v) => k === 'blocks' ? "…see above" : v, 2)}, null, 2)}</pre>
{/if}
  `
  ...
  try {
    ...
    switch (type) {
      case 'child_page':
        ...
        return h.warning('child_page', block.child_page.title, "has no content")
