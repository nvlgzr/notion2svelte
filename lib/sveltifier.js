...
function enrich(span) {
  ...
  if (span.type === 'mention' && span.mention.page?.id)
    rendered = k(rendered, slugForId(span.mention.page.id))
...
{/if}
  ...
  setSlugForId(page.id, slug(page))
  ...
  try {
    ...
    switch (type) {
      case 'child_page':
        ...
        return `<p style="background: floralwhite; padding: 1rem; margin: 0.25rem 0; border: 1px fuchsia solid; border-radius: 6px; ">⚠️ child_page <span style="color:gray; border: 2px solid pink; display: inline; padding: 3px; border-radius: 3px;">${block.child_page.title}</span>&nbsp;has no content</p>`
