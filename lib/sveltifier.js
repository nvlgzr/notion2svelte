...
export function renderPage(page, titleFieldName) {
  const getMarkedProps = (page) => {
    ...
    for (const key in allProps) {
      if (key.startsWith('§')) {
        ...
        markedProps[stripped] = plain(propType(allProps[key]));
      }
    }
    return markedProps;
  };
  ...
  const markedProps = getMarkedProps(page);
  ...
  const pageCover = ((cover || icon) && (h.cover(cover, icon) ?? '')) ?? '';
  const pathToComponents =
    markedProps.n2sLib?.length > 0 ? markedProps.n2sLib : '$lib/notion2svelte';
  let script = `\
      <script>
