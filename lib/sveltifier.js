...
const richText = (subTexts) =>
  subTexts.map((s) => decorateInlineContent(s)).join("");
...
export function renderPage(page, titleFieldName) {
  ...
  const title = env.HEAVY_PAYLOADS
    ...
    : `<Magic sourceJSON={${JSON.stringify(
        page.properties[titleFieldName],
        null,
        2
      )}}>${h.title(titleText)}</Magic>`;
...
`;
  ...
  for (let block of blocks) {
    try {
      if (block.type !== "unsupported") {
        const comp = renderBlock(block, slug, outliner);
        const notionContext = {
          page: {
            id: page.id,
            slug: slug,
        ...
        renderedBlocks += `<Magic sourceJSON={${JSON.stringify(
          block,
          null,
          2
        )}}>${comp}</Magic> ${h.br}`;
  ...
  setSlugForId(stripDashes(page.id), slug);
    ...
    if (block.has_children) {
      ...
      if (block.type === "callout") {
        ...
        try {
          ...
          throw new Error(
            `Column List #${block.id}: ${e.message}\n\ntrace → ${e.stack}`
          );
        }
    ...
    switch (type) {
      ...
      case "image":
        try {
          ...
          const renderedURL = resolveNotionImageURL(
            originalURL,
            slug,
            block.id
          );
