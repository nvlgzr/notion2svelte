  ...
  for (let block of blocks) {
    try {
      if (block.type !== 'unsupported') {
        const comp = renderBlock(block, outliner)
...
function renderBlock(block, outliner = new Outliner()) {
  ...
  if (block.has_children) {
    ...
    const renderedChildren = block.blocks.map(b => {
      return renderBlock(b, outlinerForChildren)
    }).join('')

    if (block.type === 'toggle') {
      return h.toggle(richText(block.toggle.text), renderedChildren)
    } else {
      const renderedParent = renderBlock({ ...block, has_children: false }, outliner)
