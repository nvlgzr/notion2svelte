...
export function renderPage(page, titleFieldName) {
  ...
  const title = env.HEAVY_PAYLOADS ? `<Magic sourceJSON={${JSON.stringify(pageOnly, null, 2)}}>${h.title(titleText)}</Magic>` : h.title(titleText)
  ...
  for (let block of blocks) {
    try {
      if (block.type !== 'unsupported') {
        ...
        renderedBlocks += `<Magic sourceJSON={${JSON.stringify(notionContext, null, 2)}}>${comp}</Magic> ${h.br}`
      }
    } catch (e) {
