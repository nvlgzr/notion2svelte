...
export async function fetchAllPages(options) {
  ...
  let { statusFieldName, publishableStatus, dbId } = {
    ...defaults,
    ...options
  };
  ...
  try {
    const q = {
      ...
      filter: {
        ...
        select: {
          equals: publishableStatus
        }
      }
    };
    // console.log('🦑 DB Query: ', q)
    const resource = await notion.databases.query(q);
...
export const fetchPageRoot = async (pageId) => notion.pages.retrieve({ page_id: pageId });
...
export function stripExpiryTimes(pageJSON) {
