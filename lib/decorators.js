...
export const decorateBlockContent = {
  ...
  code: (code, language) =>
    `<Code code="${escapeHtml(code)}" language="${language}" />`,
  equation: (expression) =>
...
export const decorateInlineContent = (span, keepReturns = false) => {
    ...
    if (url.match(/^\/[a-f0-9]{32}$/)) {
      ...
      return slug
        ...
        : warning(
            "k(t, url)",
            t,
            `No slug found for page id ${noLeadingSlash}`
          );
    } else {
      return url
        ? `<MagicLink href='${url}'>${t}</MagicLink>`
        : warning("k(t, url)", t, `No URL provided for ${t}`);
    }
  };
  ...
  if (span.type === "mention")
    rendered = k(
      rendered,
      slugForId(stripDashes(span.mention[span.mention.type].id))
    );
